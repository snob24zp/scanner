...
#include "cyfxsrammaster.h"
#include "cyfxgpif2config.h"

static CyU3PThread     AppThread;               /* Application thread object. */
static CyU3PDmaChannel glChHandleSRAMUtoP;      /* DMA Channel handle for U2P transfer. */
static CyU3PDmaChannel glChHandleSRAMPtoU;      /* DMA Channel handle for P2U transfer. */
        ...
        void)
{
    ...
    CyU3PReturnStatus_t     apiRetStatus = CY_U3P_SUCCESS;
    CyU3PPibClockPhase_t    core_phase, sync_phase, output_phase;
    ...
    core_phase = CYU3P_PIB_CLK_PHASE_01;        /**< Clock phase 1. Lags phase 0 by 22.5 degrees. */
    sync_phase = CYU3P_PIB_CLK_PHASE_00;        /**< Clock phase 0. No delay from master clock. */
    output_phase = CYU3P_PIB_CLK_PHASE_11;      /**< Clock phase 11. Lags phase 0 by 247.5 degrees. */

    apiRetStatus = CyU3PPibDllConfigure (CYU3P_PIB_DLL_MASTER, slave_delay, core_phase, sync_phase,
            output_phase, CyTrue);
    if (apiRetStatus != CY_U3P_SUCCESS)
    {
        CyU3PDebugPrint (CY_FX_DEBUG_PRIORITY, "Pib Dll Configuration Failed, Error Code=%d\r\n", apiRetStatus);
        CyFxAppErrorHandler (apiRetStatus);
    }
